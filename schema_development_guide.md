...
本指南將透過三個實例的對照分析，提示（Hint）您可以調整的「模組」與「流程」，協助您快速上手。

---

## 1. 核心 Schema 的三種流派對照

在開始設計前，我們先看看三種具代表性的 OpenSpec Schema 設計，以了解從「輕量」到「高度規約」的演進：

| 評估維度 | ❶ 官方純淨版 (`spec-driven`) | ❷ 官方技能橋接版 (`superpowers-bridge`) | ❸ 部門客製化版 (`fopd-sdlc`) |
| :--- | :--- | :--- | :--- |
| **設計哲學** | 輕量、工具無關、依賴少。 | 高度自動化、深度綁定 Superpowers 技能。 | **兼顧彈性與品質自省**：排除不穩定工具，強化交付品質自檢。 |
| **工作區管理** | 在當前工作區直接進行。 | 強制使用 `using-git-worktrees` 建立隔離環境。 | **開發者自管**：移除強制 Worktree 建立，交由開發者手動或以分支隔離。 |
| **實作執行器** | 由 AI 直接執行 `tasks.md` 中的粗粒度任務。 | 深度使用 `subagent-driven-development` 派發子代理。 | 使用 `subagent-driven-development` 在目前工作區自動落實 TDD。 |
| **產出文件** | 4 個：`proposal`, `specs`, `tasks`, `verify` | 7 個：新增 `brainstorm`, `design`, `plan`, `retrospective` | 7 個：在 `retrospective` 中新增 **PR 自評卡**。 |
| **適用場景** | 新專案 Scaffolding、無安裝額外 AI 技能的環境。 | 重度依賴自動化工具、偏好嚴格實作隔離的團隊。 | **企業級中大型專案**：需要 TDD 開發品質，但不希望工具鏈強制干涉 Git 結構的團隊。 |
